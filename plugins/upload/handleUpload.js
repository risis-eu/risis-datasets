...
module.exports = function handleUpload(server) {
     server.get('/accessRequest/:name', function(req, res) {
         if(!req.isAuthenticated()){
            ...
            res.render('accessRequest', {appShortTitle: appShortTitle, appFullTitle: appFullTitle, name: req.params.name, user: req.user, graphName: encodeURIComponent(req.user.graphName), resourceURI: encodeURIComponent(req.user.id)});
         }
     });
